...
class ValidWordAbbr:
    ...
    def __init__(self, dictionary: List[str]):
        self.abbreviations = {}
        self.abbreviations = defaultdict(lambda: set())
        for each_word in dictionary:
            self.abbreviations[self.get_abbreviation(each_word)].add(each_word)

    def isUnique(self, word: str) -> bool:
        abbreviation = self.get_abbreviation(word)
        return len(self.abbreviations[abbreviation]) == 0 or (len(self.abbreviations[abbreviation]) == 1 and word == list(self.abbreviations[abbreviation])[0])
